...
def start_torghost():
    ...
    command_out("brew services start tor")
    os.system('sudo fuser -k 9051/tcp > /dev/null 2>&1')
    print(bcolors.GREEN + '[done]' + bcolors.ENDC)
    print(t() + ' Starting new tor daemon '),
    command_out('sudo tor -f /etc/tor/torghostrc > /dev/null'
              )
    print(bcolors.GREEN + '[done]' + bcolors.ENDC)
...
def check_update():
    ...
    if version.parse(newversion) > version.parse(VERSION):
        ...
        yes = {'yes', 'y', 'ye', 'yeh', 'yuh', 'I think so...'}
        no = {'no', 'n'}
        ...
        if choice in yes:
            os.system(
                'cd /tmp && git clone https://github.com/MarquisTheCoder/torghost-mac-port')
